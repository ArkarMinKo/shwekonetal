...
function createAdmin(req, res) {
  ...
  generateAdminId(db, (idErr, newId) => {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        // If role is 'owner', check if an owner already exists
        if (roleStr === "owner") {
          const checkOwnerSql = "SELECT COUNT(*) AS count FROM admin WHERE role = 'owner'";
          return db.query(checkOwnerSql, async (err, results) => {
            if (err) {
              res.statusCode = 500;
              return res.end(JSON.stringify({ error: err.message }));
            }
            ...
            if (results[0].count > 0) {
              ...
              return res.end(JSON.stringify({ error: "Owner account already exists. Cannot create another owner." }));
            }

            // No owner yet — proceed with creation
            createNewAdmin();
          });
        } else {
          createNewAdmin();
