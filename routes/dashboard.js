                ...
                db.query(todayTransactionsSql, [date], (err, todayTransactionsResult) => {
                    ...
                    db.query(usersSql, (err, usersResult) => {
                        ...
                        db.query(getLatestFormulaSql, (err, formulaResult) => {
                            ...
                            const todayUserCount = usersResult.filter(user => user.create_at === date && user.status === 'approved').length;
...
function buyingPricesChart(req, res) {
  ...
  db.query(sql, (err, rows) => {
    ...
    const allDatesSorted = rows.map(r => r.date).sort();
...
function topWallet(req, res) {
  const usersSql = `
    SELECT state, city, fullname AS user, gold
    FROM users
    ORDER BY gold DESC
    LIMIT 3
  `;
